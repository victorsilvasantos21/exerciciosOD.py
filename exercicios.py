...
Implemente um programa em Python no qual o usuário deverá informar o nome e o poder (0 a 10) de três personagens. 
...
A classe deverá possuir dois dados para representar as características do animal.

#3
Os quatro pilares do paradigma de Orientação a Objetos são:

A - Sequenciamento, Procedimentos, Bibliotecas e Herança.
(x)B - Herança, Polimorfismo, Classes e Objetos.
...
===========================================
#5
Crie uma classe para representar um item de cenário, com os atributos de:
    * descrição (string), 
    * data de criação (date) e
    * altura (float). 

Crie o construtor básico e também um método para imprimir 
todos dados de um item de cenário. 

Por fim, crie um método para calcular a quanto tempo o item foi criado, além de um programa em Python para testar
a classe criada.

#6
Crie uma classe para representar um objeto em um Plano Cartesiano. Todo objeto no plano cartesiano
tem como atributos 'x' e 'y'. Dessa forma, crie  o construtor básico e um método para exibir um objeto (x,y).

Por fim, crie um programa em Python que cadastre 'n' objetos em uma lista (List), um método de que exiba
os objetos da lista.